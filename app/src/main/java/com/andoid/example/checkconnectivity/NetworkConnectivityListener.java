...
/**
 * Created by antlap on 25/10/2017.
 */
...
public class NetworkConnectivityListener {
    ...
    /**
     * This methods registers a Handler to be called back onto with the specified clientId code when
     * the network connectivity state changes.
     *
     * @param target The target handler.
     * @param clientId   The clientId code to be used when posting a message to the handler.
     */
    public void registerHandler(Handler target, int clientId) {
        mHandlers.put(target, clientId);
    }
    ...
    private class ConnectivityBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (!action.equals(ConnectivityManager.CONNECTIVITY_ACTION) ||
                    mListening == false) {
                Log.w(LOG_TAG, "onReceived() called with " + mState.toString() + " and " + intent);
                return;
            }
            ...
            while (iterator.hasNext()) {
                ...
                Integer clientId = mHandlers.get(handler);
                Message message = Message.obtain(handler, clientId);
